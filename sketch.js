...
function preload() {
  ...
  ghostSound = loadSound("assets/ghostsound.m4a")
}
...
function setup() {
  ...
  bridge = new Bridge(20, { x: 50, y: height / 2 - 110 });
  ...
  for (var i = 0; i <= 10; i++) {
    ...
    var stone = new Stone(x, y, 100, 120);
    stones.push(stone);
  }
  ...
  ghost = createSprite(200,75,200,200)
...
function draw() {
  ...
  if (ghost.position.x >= width - 100 && !collided) {
    ghost.velocityX = -10;
  }

  if (ghost.position.x <= 100 && !collided) {
    ghost.velocityX = 10;
  }
